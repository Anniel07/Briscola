 ...
 * class, but subclasses of Hub can override it to do extra setup or checking
...
public class Hub {
	...
	/**
	 * A queue of messages received from clients. When a message is received, it is
	 * placed in this queue. A separate thread takes messages from the queue and
	 * processes them (in the order in which they were received).
	 */
	private LinkedBlockingQueue<Message> incomingMessages;

	/**
	 * If the autoreset property is set to true, then the ObjectOutputStreams that
	 * are used for transmitting messages to clients are reset before each object is
	 * sent.
	 */
	 ...
	 * @throws IOException if it is not possible to create a listening socket on the
	 *                     specified port.
	 */
	...
	/**
	 * This method is called when a message is received from one of the connected
	 * players. The method in this class simply wraps the message, along with the ID
	 * of the sender of the message, into a message of type ForwardedMessage and
	 * then sends that ForwardedMessage to all connected players, including the one
	 * who sent the original message. This behavior will often be overridden in
	 * subclasses.
	 * 
	 ...
	 * getPlayerList() can be called to get a list of connected players. The method
	 ...
	 * extra checking or set up before the connection is fully established. It is
	 * called after the playerID has been transmitted to the client. If this method
	 ...
	 * @param out      a stream to which message to the client can be written. After
	 ...
	 * 
	 * @return an array containing the ID numbers of all the connected clients. The
	 *         array is newly created each time this method is called.
	 */
	...
	/**
	 * Stops listening, without disconnecting any currently connected clients. You
	 * might do this, for example, if some maximum number of player connections has
	...
	/**
	 * Restarts listening and accepting new clients. This would only be used if the
	 * shutDownHub() method has been called previously.
	 * 
	 * @param port the port on which the server should listen.
	 * @throws IOException if it is impossible to create a listening socket on the
	 *                     specified port.
	 */
	 ...
	 * 
	 * @param message the message to be sent to all connected clients. This object
	 ...
	 * 
	 * @param recipientID the ID number of the player to whom the message is to be
	 *                    sent. If there is no such player, then the method returns
	 *                    the value false.
	 * @param message     the message to be sent to all connected clients. This
	 *                    object must implement the Serializable interface. Messages
	 *                    must not be null.
	 ...
	 * more information on resetting the output stream. The default value is false.
	 */
	...
	private class ConnectionToClient { // Handles communication with one client.
		 ...
		 * that's done, it creates a thread for receiving incoming messages, and goes
		 * into an infinite loop in which it transmits outgoing messages.
		 */
		...
		/**
		 * The ReceiveThread reads messages transmitted from the client. Messages are
		 * dropped into an incomingMessages queue, which is shared by all clients. If a
		 * DisconnectMessage is received, however, it is a signal from the client that
